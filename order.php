...
<div class="col-lg-9 mt-2">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <?php
            // Ambil data pesanan yang statusnya 'Menunggu'
            $query = "SELECT * FROM orders WHERE status = 'Menunggu' ORDER BY DATE(waktu_order) DESC, waktu_order DESC";  // Mengambil semua pesanan yang statusnya 'Menunggu', diurutkan berdasarkan tanggal dan waktu
            if (isset($_POST['search'])) {
                $nama_pelanggan = $_POST['nama_pelanggan'];
                $query = "SELECT * FROM orders WHERE status = 'Menunggu' AND pelanggan LIKE '%$nama_pelanggan%' ORDER BY DATE(waktu_order) DESC, waktu_order DESC";
            }
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    if ($order_date != $previous_date) {
                        ...
                        // Tampilkan kolom aksi hanya jika level >= 2
                        if ($user_level >= 2) {
                            echo '<th>Aksi</th>';
                        }
